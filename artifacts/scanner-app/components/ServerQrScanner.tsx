...
type ServerQrScannerProps = {
  visible: boolean;
  onCancel: () => void;
  onPairing: (input: { url: string; pairingToken: string; deviceLabel?: string }) => Promise<void> | void;
};
...
function parsePairing(rawValue: string): { url: string; pairingToken: string; deviceLabel?: string } {
  const trimmed = rawValue.trim();
  if (!trimmed.startsWith("{")) {
    throw new Error("Scan a pairing QR generated by the Inventory Scanner desktop app.");
  }
  const payload = JSON.parse(trimmed) as ServerQrPayload;
  if (
    payload.type !== "inventory-scanner/scanner-pairing"
    || typeof payload.url !== "string"
    || typeof payload.pairingToken !== "string"
  ) throw new Error("This is not an Inventory Scanner pairing QR code.");
...
export default function ServerQrScanner({
  ...
  const handleBarcodeScanned = async ({ data }: BarcodeScanningResult) => {
    ...
    try {
      await onPairing(parsePairing(data));
    } catch (error) {
