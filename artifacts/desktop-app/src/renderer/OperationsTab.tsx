...
function ReturnsAndScrap({ stores, planning, operations, busy, permissions, run }: { stores: StoresState; planning: PlanningState; operations: OperationsState; busy: boolean; permissions: Permission[]; run: (action: () => Promise<unknown>, message: string) => Promise<void> }) {
  ...
  const [requirementDisposition, setRequirementDisposition] = useState<"STILL_REQUIRED" | "REQUIREMENT_REDUCED" | "ORDER_CANCELLED">("STILL_REQUIRED");
  const [reference, setReference] = useState("");
  ...
  async function submit() {
    if (mode === "production") {
      await window.desktop.operations.productionReturn({ clientTransactionId: clientId("production-return"), tallyItemGuid: item, quantity, originalMovementId: reference || undefined, productOrderId: productOrderId || undefined, lotId: lotId || undefined, targetCondition: target, requirementDisposition, explanation: reference ? undefined : notes, eventDate: transactionDate || undefined, serialNumbers: serials(serialText) });
  ...
  return <div className="planning-section-stack">
    <OperationPanel eyebrow="RETURNS & WRITE-OFF" title="Choose a distinct inventory workflow">
      ...
      {modes.length > 0 ? <form className="operations-form" onSubmit={(event) => { event.preventDefault(); void run(submit, `${label(mode)} recorded.`); }}>
        ...
        {mode === "production" && <><label>Original issue movement<input placeholder="Optional movement ID" value={reference} onChange={(event) => setReference(event.target.value)} /></label><label>Product order<select value={productOrderId} onChange={(event) => setProductOrderId(event.target.value)}><option value="">Unlinked</option>{planning.productOrders.map((entry) => <option key={entry.id} value={entry.id}>{entry.externalReference} · {entry.productName}</option>)}</select></label><label>Requirement disposition<select value={requirementDisposition} onChange={(event) => setRequirementDisposition(event.target.value as typeof requirementDisposition)}><option value="STILL_REQUIRED">Still required</option><option value="REQUIREMENT_REDUCED">Requirement reduced</option><option value="ORDER_CANCELLED">Order cancelled</option></select></label></>}
...
function Production({ stores, planning, operations, busy, run }: { stores: StoresState; planning: PlanningState; operations: OperationsState; busy: boolean; run: (action: () => Promise<unknown>, message: string) => Promise<void> }) {
  const productionOrders = planning.productOrders.filter((order) => order.orderType === "PRODUCTION");
  const [orderId, setOrderId] = useState(productionOrders.find((order) => !["COMPLETED","CANCELLED"].includes(order.status))?.id ?? "");
  ...
  return <div className="planning-section-stack">
    <OperationPanel eyebrow="PRODUCT ORDER EXECUTION" title="Release, issue, return, scrap, and complete">
      <div className="operations-filters"><label>Product order<select value={orderId} onChange={(event) => { const next = event.target.value; setOrderId(next); const selected = productionOrders.find((entry) => entry.id === next); setDestination(selected?.productTallyGuid ?? ""); setFinishedItem(selected?.productTallyGuid ?? ""); }}><option value="">Select…</option>{productionOrders.map((entry) => <option key={entry.id} value={entry.id}>{entry.externalReference} · {entry.productName} × {entry.quantity}</option>)}</select></label>{order && <div className="operations-order-summary"><strong>{order.productName}</strong><span>{order.quantity} planned · due {order.requiredDate || "not set"}</span><Status value={execution?.status ?? order.status} /></div>}</div>
