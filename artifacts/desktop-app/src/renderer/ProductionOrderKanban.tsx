...
export default function ProductionOrderKanban({ planning, stores, canManage, onRefresh, onNotice, onError }: Props) {
  ...
  const visibleOrders = planning.productOrders.filter((order) => {
    ...
    return (!search || haystack.includes(search.toLocaleLowerCase()))
      ...
      && (showClosed || !["COMPLETED", "CANCELLED"].includes(order.status));
  });
  ...
  return <div className="production-board-page">
    <div className="production-board-summary">
      <div><span>Visible orders</span><strong>{visibleOrders.length}</strong></div>
      <div><span>Order value</span><strong>{money(totalValue)}</strong></div>
    ...
    <div className="production-board-toolbar">
      <input className="production-board-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search PO, organisation, product, owner…" />
      ...
      <label className="check-row production-board-closed"><input type="checkbox" checked={showClosed} onChange={(event) => setShowClosed(event.target.checked)} />Show closed</label>
      {canManage && <><button className="button button--secondary" type="button" onClick={() => fileInput.current?.click()}>Import tracker</button><input ref={fileInput} hidden type="file" accept=".xlsx,.xls,.csv" onChange={(event) => { const file = event.target.files?.[0]; if (file) void parseTracker(file); event.currentTarget.value = ""; }} /><button className="button button--secondary" type="button" onClick={() => setSettingsOpen(true)}>Fields & states</button><button className="button" type="button" onClick={() => setDraft({ ...emptyOrder(), workflowStateId: planning.productOrderWorkflowStates[0]?.id ?? "" })}>Add order</button></>}
    </div>
    ...
    <div className="production-kanban" aria-label="Production order workflow board">
          ...
          <div className="production-kanban-cards">
            {orders.map((order) => {
              ...
              >
                ...
                <div className="production-card-meta"><span>Qty <strong>{order.quantity}</strong>{order.pendingQuantity != null && <> · {order.pendingQuantity} pending</>}</span><span className={due.overdue ? "danger-text" : ""}>{due.text}</span></div>
                <div className="production-card-tags"><span className={`order-feasibility order-feasibility--${order.feasibility.toLocaleLowerCase().replaceAll("_", "-")}`}>{order.feasibility.replaceAll("_", " ")}</span><span className="tally-link-badge">Tally linked</span></div>
                {(order.taskRemarks || order.pendingMaterial || order.rawMaterialToOrder) && <p className="production-card-task">{order.taskRemarks || order.pendingMaterial || order.rawMaterialToOrder}</p>}
                <footer><span>{order.responsiblePerson || "Unassigned"}</span><span>{order.crfStatus ? `CRF: ${order.crfStatus}` : order.bomVersionLabel}</span></footer>
              </article>;
            })}
            {orders.length === 0 && <div className="production-column-empty">Drop orders here</div>}
          </div>
        </section>;
      })}
    </div>
    ...
    {draft && <div className="production-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setDraft(null); }}>
      <section className="production-modal" role="dialog" aria-modal="true" aria-label={draft.id ? "Edit production order" : "Add production order"}>
        <header><div><p className="eyebrow">{draft.id ? "ORDER DETAIL" : "NEW ORDER"}</p><h2>{draft.id ? draft.externalReference || "Production order" : "Add production order"}</h2></div><button className="icon-button" type="button" onClick={() => setDraft(null)} aria-label="Close">×</button></header>
        <div className="production-modal-body">
          <div className="production-form-grid">
            ...
            <label className="production-form-wide">Product / Tally Stock Item<select value={draft.productTallyGuid} onChange={(event) => setDraft({ ...draft, productTallyGuid: event.target.value })}><option value="">Select product…</option>{products.map((product) => <option key={product.tallyGuid} value={product.tallyGuid}>{product.name} · {product.parentName}</option>)}</select></label>
        ...
        <footer><button className="button button--secondary" type="button" onClick={() => setDraft(null)}>Cancel</button><button className="button" disabled={busy || !canManage || !draft.productTallyGuid} type="button" onClick={() => void saveOrder()}>Save order</button></footer>
      </section>
    </div>}
